...
QByteArray QAESEncryption::expandKey(const QByteArray &key,
  ...
  if (m_aesNIAvailable) {
    ...
    case AES_128: {
      ...
      AES_KEY aesKey;
      if (isEncryptionKey) {
        AES_set_encrypt_key((unsigned char *)key.constData(),
                            aes128.userKeySize, &aesKey);
      } else {
        AES_set_decrypt_key((unsigned char *)key.constData(),
                            aes128.userKeySize, &aesKey);
      }
  ...
  case ECB: {
    ...
    if (m_aesNIAvailable) {
      ...
      AES_ECB_encrypt((unsigned char *)alignedText.constData(),
                      (unsigned char *)outText.data(), alignedText.size(),
                      expKey, m_nr);
      return outText;
    }
#endif
  ...
  case CBC: {
    ...
    if (m_aesNIAvailable) {
      ...
      AES_CBC_encrypt((unsigned char *)alignedText.constData(),
                      (unsigned char *)outText.data(), ivec, alignedText.size(),
                      expKey, m_nr);
      return outText;
    }
#endif
...
QByteArray QAESEncryption::decode(const QByteArray &rawText,
                                  const QByteArray &key, const QByteArray &iv) {
  if ((m_mode >= CBC && (iv.isEmpty() || iv.size() != m_blocklen)) ||
      key.size() != m_keyLen || rawText.size() % m_blocklen != 0)
    return QByteArray();
  ...
  // the expandedKeys aren't the same for !aes-ni! ENcryption and DEcryption
  // (only CBC and EBC) but if you are !NOT! using aes-ni then the expandedKeys
  // for encryption and decryption are the SAME!!!
  ...
  case ECB:
    ...
    if (m_aesNIAvailable) {
      ...
      AES_ECB_decrypt((unsigned char *)rawText.constData(),
                      (unsigned char *)ret.data(), rawText.size(), expKey,
                      m_nr);
      break;
    }
#endif
  ...
  case CBC:
    ...
    if (m_aesNIAvailable) {
      ...
      AES_CBC_decrypt((unsigned char *)rawText.constData(),
                      (unsigned char *)ret.data(), ivec, rawText.size(), expKey,
                      m_nr);
      break;
    }
#endif
