...
public:
   ...
   * \param key:      user-key (key.size either 128, 192, 256 bits depending on
   * AES::Aes)
  ...
  Decrypt(QAESEncryption::Aes level, QAESEncryption::Mode mode,
          ...
          QAESEncryption::Padding padding = QAESEncryption::ISO);
  /*!
   * \brief static method call to expand the user key to fit the
   * encrypting/decrypting algorithm
   * \param level:            AES::Aes level
   * \param mode:             AES::Mode mode
   * \param key:              user-key (key.size either 128, 192, 256 bits
   * depending on AES::Aes)
   * \param expKey:           output expanded key
   * \param isEncryptionKey:    always 'true' || only 'false' when DECRYPTING in
   * CBC or EBC mode with aesni (check if supported)
  ...
  /*!
   * \brief object method call to expand the user key to fit the
   * encrypting/decrypting algorithm
   * \param key:              user-key (key.size either 128, 192, 256 bits
   * depending on AES::Aes)
   * \param isEncryptionKey:    always 'true' || only 'false' when DECRYPTING in
